...
export async function interactWithAiCopilotAction(
  ...
  const balancedScheduleInstruction =
    '专业均衡排产规则：生成排产草案时必须先统一识别交期格式（如 2026-05-15、2026/05/15、5月11日、5/11、05-11）；交期无法识别的订单必须列为数据不完整，不能进入严格交期排产草案。必须严格按交期从早到晚；不允许后交期订单排到前交期订单前面；同一天交期内，工时高的订单优先；先计算本周候选订单总工时，再用总工时除以 6 得到日均目标；每天负荷尽量围绕日均目标上下浮动 500 分钟；大单可以造成单日适度超出，但不能把大量订单堆到某一天；不允许前几天低负荷、最后一天严重爆仓；不允许周六负荷远高于其他天。如果交期顺序和负荷均衡冲突，必须优先保证交期顺序并说明原因，不能为了均衡提前后交期订单。';
  const system =
